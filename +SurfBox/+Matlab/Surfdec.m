...
if ~nomex
    ...
    dec_flts = SurfBox.Matlab.get_cbd_filters_load(bo, 'd');
    rec_flts = SurfBox.Matlab.get_cbd_filters_load(bo, 'r');

    %% Call the mex function
    Y = SurfBox.Matlab.mexSurfaceletDec(X, Pyr_mode, Lev_array_int, dec_flts, rec_flts, msize, beta, lambda);
...
subs = SurfBox.Matlab.PyrNDDec_mm(X, 'F', L_NDFB, Pyr_mode, smooth_func);
...
for n = 1 : L_NDFB
    if all(Lev_array{n} == 0)
        ...
        [Y_sub, Recinfo_sub] = SurfBox.Matlab.NDFBdec(subs{n}, 'F', 'S', 'C', Lev_array{n}, HGfname, 'bo', bo, 'nstage', nstage, 'norder', norder);
        subs{n} = [];
        Y{n} = Y_sub;
        clear Y_sub;

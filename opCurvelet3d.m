...
%
%   opCurvelet(M,N,O,NBSCALES,NBANGLES) creates a three-dimensional
%   curvelet operator for M by N by O matrices. The curvelet transform is
%   computed using the Curvelab code.
%
%   The remaining two parameters are optional; NBSCALES gives the
%   number of scales and is set to max(1,ceil(log2(min(M,N,O)) - 3)) by
...
%   http://www.cs.ubc.ca/labs/scl/spot
    ...
    properties (SetAccess = protected)
       nbscales;
       nbangles;
       ac = 1;   %currently not adjustable, but put it here for view
       hdr;          %sizes of coefficient vectors
       cn;           %total number of coefficients
       sz;           %size of curvelet
    ...
    methods
       ...
       function op = opCurvelet3d(m,n,p,nbscales,nbangles)
          ...
          [tmphdr, cn] = fdct3d_sizes_mex(m,n,p,nbscales,nbangles,ac); %#ok<PROP>
          hdr = cell(nbscales,1);
          hdr{1} = {[tmphdr{1:3}]};
          for i = 2:nbscales
             j = 4 + 10*(i - 2);
    ...
    methods ( Access = protected )
       ...
       function x = multiply(op,x,mode)
          ...
          if mode == 1
            % Analysis mode
            x = fdct3d_forward_mex(op.sz(1),op.sz(2),op.sz(3),...
               op.nbscales,op.nbangles,op.ac,reshape(x,op.sz));
            x = fdct_wrapping_c2r(x);
            x = spot.utils.fdct_c2v(x,op.cn);
         else
            % Synthesis mode  
            x = spot.utils.fdct3d_v2c(x,op.hdr);
            x = fdct_wrapping_r2c(x);
            x = fdct3d_inverse_mex(op.sz(1),op.sz(2),op.sz(3),...
               op.nbscales,op.nbangles,op.ac,x);
            x = real(x);
            x = x(:);
         end
       end % Multiply          
